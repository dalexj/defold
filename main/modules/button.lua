...
function buttons_init(self, ids)
	...
	for _, button_id in pairs(ids) do
		self.buttons[button_id] = gui.get_node(button_id .. "/bg")
...
function buttons_init_clone(self, proto_id, number_of_buttons, position_fn, text_fn)
	...
	for i = 1, number_of_buttons, 1 do
		...
		gui.set_position(new_nodes[hash(proto_id .. "/bg")], position_fn(i))
		gui.set_text(new_nodes[hash(proto_id .."/text")], text_fn(i))
		self.buttons[i] = new_nodes[hash(proto_id .. "/bg")]
...
function buttons_on_input(self, action_id, action, click_fn)
	...
	for key, button_node in pairs(self.buttons) do
		local new_state = "NORMAL"
		if gui.is_enabled(button_node) and gui.pick_node(button_node, action.x, action.y) then
		...
		if not self.buttons_metadata[key].skip_effects and new_state ~= self.buttons_metadata[key].state then
			...
			gui.set_color(button_node, new_color)
		end
	end
